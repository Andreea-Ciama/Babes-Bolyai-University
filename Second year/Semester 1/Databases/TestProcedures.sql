...
GO
CREATE OR ALTER VIEW producersWithMoreThanTwoMovies AS
	SELECT P.FirstName, P.NrOfMovies
	FROM Producers P
	WHERE P.NrOfMovies > 2

--  a view with a SELECT statement operating on at least 2 tables
GO
CREATE OR ALTER VIEW heroesAndGActors AS
	SELECT H.Name, G.HID
	FROM Heroes H INNER JOIN GActors G on G.HID = H.HID

-- a view with a SELECT statement that has a GROUP BY clause and operates on at least 2 tables
GO
CREATE OR ALTER VIEW groupHeroesByFans AS
	SELECT H.HID, H.Name, COUNT(*) AS fans
	FROM Heroes H
	INNER JOIN Fan_of_hero FH ON H.HID = FH.HID
...
-- a view with a SELECT statement that has a GROUP BY clause and operates on at least 2 tables
EXEC addToViews 'groupHeroesByFans'
EXEC addToTests 'Test1'
EXEC addToTables 'Heroes'
EXEC connectTableToTest 'Heroes', 'Test1', 500, 1
EXEC addToTables 'GActors'
EXEC connectTableToTest 'Singer', 'Test1', 500, 2
EXEC addToTables 'Fan'
EXEC connectTableToTest 'Fan', 'Test1', 500, 3
EXEC addToTables 'Fan_of_hero'
EXEC connectTableToTest 'Fan_of_hero', 'Test1', 250, 4
EXEC insertDataIntoTable 10, 1, 4
EXEC connectViewToTest 'groupHeroesByFans', 'Test1' 

-- second test
-- a table with a single-column primary key 
-- a view with a SELECT statement operating on one table
EXEC addToViews 'producersWithMoreThanTwoMovies'
EXEC addToTests 'Test2'
EXEC addToTables 'Producers'
EXEC connectTableToTest 'Producers', 'Test2', 500, 1
EXEC connectViewToTest 'producersWithMoreThanTwoMovies', 'Test2'
...
EXEC addToViews 'heroesAndGActors'
EXEC addToTests 'Test3'
EXEC addToTables 'Heroes'
EXEC connectTableToTest 'Heroes', 'Test3', 500, 1
EXEC addToTables 'GActors'
EXEC connectTableToTest 'GActors', 'Test3', 500, 2
EXEC connectViewToTest 'heroesAndGActors', 'Test3'

EXEC runAllTests

EXEC runTest 2, 'Test results for test with the ID 2'
