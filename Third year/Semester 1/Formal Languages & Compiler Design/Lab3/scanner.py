...
class Scanner:
    ...
    def read_tokens(self):
        with open('token.in', 'r') as f:
            for i in range(19):  # Adjust the range based on the number of tokens in your file
                token = f.readline().strip()
                if token == 'space':
                    self.__separators.append(" ")
                else:
                    self.__reserved_words.append(token)
        # Add special symbols to the reserved words
            self.__reserved_words.extend(['{', '}'])
            for i in range(17):
